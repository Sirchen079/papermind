...
PaperMind 是本地运行的论文阅读与研究资料管理应用，支持多个研究项目、论文问答、专题知识、笔记和研究成果导出。数据保存在本机，兼容多家模型服务。当前版本 0.5.4，提供 Windows 安装版与便携版。
...
论文整理、阅读与人工编辑可完全离线使用。AI 功能请同时配置文本模型和 embedding 向量模型；通用多模态模型可兼任对话、翻译、OCR 与大模型重排序，向量模型负责检索召回。发送给模型的内容由所选资料和当前功能决定，请按资料的保密要求选择模型服务。

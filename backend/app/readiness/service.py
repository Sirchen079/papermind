...
def _enabled_model_roles(session: Session) -> set[str]:
    ...
    for _,provider in rows:
        if provider.id not in enabled:
            try:enabled[provider.id]=resolve(provider)[0].enabled and not provider.is_deleted
            except LookupError:enabled[provider.id]=False
    return {model.role_default for model,provider in rows if model.role_default and enabled[provider.id]}

...
def list_recently_read(session: Session, limit: int) -> list[dict]:
    """研究主页「继续阅读」数据：按 last_read_at 降序的已读论文。

    排除软删除论文和从未打开过（last_read_at 为空）的阅读状态；limit 由
    API 层校验（1..50），这里再做一次防御性下限保护。
    """
    if limit < 1:
        raise ValueError("limit must be a positive integer")
    rows = session.exec(
        select(PaperReadingState, Paper)
        .join(Paper, PaperReadingState.paper_id == Paper.id)
        .where(Paper.is_deleted == False, PaperReadingState.last_read_at != None)  # noqa: E712
        .order_by(PaperReadingState.last_read_at.desc())
        .limit(limit)
    ).all()
    return [
        {
...
def patch_reading_state(session: Session, paper_id: int, payload: dict) -> dict:
    ...
    if "last_page" in payload:
        # P11.5: reader progress — positive int or None (like excerpt.page).
        last_page = payload["last_page"]
        if last_page is not None and (not isinstance(last_page, int) or last_page <= 0):
            raise ValueError("last_page must be a positive integer")
        row.last_page = last_page
        if last_page is not None:
            # Opening/turning a PDF page is reading even without a manual status change.
            row.last_read_at = utcnow()
            if row.started_at is None:
                row.started_at = row.last_read_at

...
def select_for_chat(session: Session, user_message: str, manual_ids: list[int] | None = None) -> list[Skill]:
    ...
    for s in session.exec(select(Skill).where(Skill.enabled == True).order_by(Skill.id)).all():  # noqa: E712
        ...
        if s.trigger == "auto":
            out.append(s)
        elif s.trigger == "manual" and s.id in requested:
            out.append(s)
    ...
    if requested - {s.id for s in out}:
        raise ValueError("所选技能已停用、被删除或不支持问答，请重新选择。")
    return out

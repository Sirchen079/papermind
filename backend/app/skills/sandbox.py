...
def run_tool(
    ...
    with tempfile.TemporaryDirectory(prefix="papermind-skill-") as workdir:
        ...
        env = _sandbox_env({"PAPERMIND_CONTEXT_FILE": str(work / "context.json"), "PYTHONIOENCODING": "utf-8"})
        try:
            proc = subprocess.run(
                ([sys.executable, "--run-skill", str(script)] if getattr(sys, "frozen", False)
            ...
            if getattr(sys, "frozen", False) and not (work / "stdout.txt").exists():
                return ToolResult(False, "", "技能执行进程没有返回结果，请检查程序安装是否完整。", -1, int((time.monotonic() - start) * 1000))
            return ToolResult(
                ok=proc.returncode == 0,
                stdout=(work / "stdout.txt").read_text(encoding="utf-8") if (work / "stdout.txt").exists() else proc.stdout,
                stderr=(work / "stderr.txt").read_text(encoding="utf-8") if (work / "stderr.txt").exists() else proc.stderr,

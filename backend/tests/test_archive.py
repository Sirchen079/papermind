...
def _create_backup_filename() -> str:
    ...
    with Session(get_engine()) as session:
        return create_backup(session)["filename"]


def test_download_backup_without_token_is_403(client, local_token):
    filename = _create_backup_filename()

    resp = client.get(f"/api/archive/backups/{filename}")

    assert resp.status_code == 403
    assert resp.json()["detail"] == "local token required"


def test_download_backup_with_wrong_token_is_403(client, local_token):
    filename = _create_backup_filename()

    resp = client.get(
        f"/api/archive/backups/{filename}",

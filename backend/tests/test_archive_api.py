...
@pytest.fixture
def local_token(env):
    # reset_token_cache 对齐 test_archive.py：lru_cache 进程级共享，
...
def test_backup_api_creates_lists_downloads_and_rejects_traversal(client, env, local_token):
    ...
    traversal = client.get(
        "/api/archive/backups/..%2Fmaster.key",
        headers={"X-Local-Token": local_token},
    )

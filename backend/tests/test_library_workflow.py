...
def test_master_research_workflow_http_smoke(client):
    ...
    root = client.post("/api/thesis/projects", json={"name": "科研论文主线", "kind": "direction"})
    ...
    with Session(get_engine()) as session:
        ...
        agent_collection = json.loads(
            t_add_paper_to_collection(session, paper_id, "研究会议讨论", "下次组会需要回顾")
        )
    ...
    library_row = next(row for row in library_rows.json()["items"] if row["id"] == paper_id)
    ...
    assert exported["projects"][0]["name"] == "科研论文主线"

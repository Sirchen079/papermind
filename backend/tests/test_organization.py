...
def test_organization_models_roundtrip(tmp_path):
    ...
    with Session(eng) as session:
        ...
        collection = Collection(name="研究论文必读", description="论文写作阶段反复阅读")
        session.add(paper)
        session.add(tag)
    ...
    assert [row.name for row in collections] == ["研究论文必读"]
...
def test_tags_api_assigns_tags_and_papers_list_includes_them(client):
    ...
    paper_row = next(row for row in client.get("/api/papers").json()["items"] if row["id"] == paper_id)
...
def test_collections_api_membership_and_export(client):
    ...
    created = client.post(
        "/api/collections",
        json={"name": "研究论文必读", "description": "论文写作阶段反复阅读"},
    )
    assert created.status_code == 201
    ...
    paper_row = next(row for row in client.get("/api/papers").json()["items"] if row["id"] == paper_id)
    assert paper_row["collections"] == [{"id": collection_id, "name": "研究论文必读"}]
    ...
    assert exported["collections"][0]["name"] == "研究论文必读"

...
# --- SSRF 出站 URL 守卫（ensure_http_url）--------------------------------------
# base_url 来自用户输入，凡由它构造出站 URL 的路径都必须在请求前校验
# scheme http/https 且有 host，否则直接 ValueError（调用方已有 502 包装）。
# 用 MagicMock 顶替真实出口（httpx.get / litellm.*），断言校验先于任何网络调用。

def _plain_provider(ptype, base_url):
    """Un-persisted Provider row — enough for URL-construction paths."""
    return Provider(id=1, name="bad", type=ptype, base_url=base_url, api_key_encrypted=None)


def test_list_models_rejects_non_http_base_url(monkeypatch):
    """ftp:// base_url 不得触发任何网络请求，必须在构造 URL 时即抛 ValueError。"""
    # list_models 的出站 GET 已切换到 validated_get（app.security.url_guard），
    # tripwire 随之移到 url_guard 的 httpx.get 出口处。
    tripwire = MagicMock(name="httpx_get")
    monkeypatch.setattr("app.security.url_guard.httpx.get", tripwire)
    client = ProviderClient(session_factory=MagicMock(), crypto=MagicMock())
    provider = _plain_provider("openai_compat", "ftp://internal-host/v1")
    with pytest.raises(ValueError):
        client.list_models(provider)
    tripwire.assert_not_called()


def test_models_endpoint_rejects_non_http_base_url():
    """_models_endpoint 是 list_models 的 URL 构造 helper，同样必须拒绝非 http(s)。"""
    client = ProviderClient(session_factory=MagicMock(), crypto=MagicMock())
    with pytest.raises(ValueError):
        client._models_endpoint(_plain_provider("openai_compat", "ftp://internal-host/v1"))
    with pytest.raises(ValueError):
        client._models_endpoint(_plain_provider("anthropic", "ftp://internal-host"))


def test_complete_rejects_non_http_base_url(monkeypatch):
    """chat 端点构造（route_completion -> api_base）必须在 litellm 请求前校验。"""
    tripwire = MagicMock(name="litellm_completion")
    monkeypatch.setattr("app.providers.client.litellm.completion", tripwire)
    client = ProviderClient(session_factory=MagicMock(), crypto=MagicMock())
    provider = _plain_provider("openai_compat", "ftp://internal-host/v1")
    with pytest.raises(ValueError):
        client.complete(
            provider, "m", [{"role": "user", "content": "hi"}], request_kind="chat"
        )
    tripwire.assert_not_called()


def test_embed_rejects_non_http_base_url(monkeypatch):
    """embeddings 端点构造（api_base=provider.base_url）同样必须请求前校验。"""
    tripwire = MagicMock(name="litellm_embedding")
    monkeypatch.setattr("app.providers.client.litellm.embedding", tripwire)
    client = ProviderClient(session_factory=MagicMock(), crypto=MagicMock())
    provider = _plain_provider("openai_compat", "ftp://internal-host/v1")
    with pytest.raises(ValueError):
        client.embed(provider, "m", ["text"])
    tripwire.assert_not_called()

...
from app.models import Model, Paper, PaperReadingState, Provider, ReviewMatrixEntry, Summary
...
def test_papers_list_includes_reading_summary(client):
    ...
    rows = client.get("/api/papers").json()["items"]
...
def test_reader_last_page_roundtrip(client):
    """P11.5: 翻页进度写回后，重开阅读器能恢复到上次页码。"""
    with Session(get_engine()) as session:
        paper = _paper(session, "Progress Paper")
        pid = paper.id

    # 没读过：无进度
    ws = client.get(f"/api/papers/{pid}/reading").json()
    assert ws["state"]["last_page"] is None

    # 阅读器节流写回
    resp = client.patch(f"/api/papers/{pid}/reading/state", json={"last_page": 5})
    assert resp.status_code == 200
    assert resp.json()["last_page"] == 5

    # 重开恢复
    ws = client.get(f"/api/papers/{pid}/reading").json()
    assert ws["state"]["last_page"] == 5

    # 允许显式清除
    resp = client.patch(f"/api/papers/{pid}/reading/state", json={"last_page": None})
    assert resp.status_code == 200
    assert resp.json()["last_page"] is None


def test_reader_last_page_validation(client):
    """P11.5: 非法 last_page（<=0 / 非整数）返回 422。"""
    with Session(get_engine()) as session:
        paper = _paper(session, "Progress Invalid")
        pid = paper.id

    for bad in (0, -3, "7", 1.5):
        resp = client.patch(f"/api/papers/{pid}/reading/state", json={"last_page": bad})
        assert resp.status_code == 422, f"last_page={bad!r} should be 422"

    ws = client.get(f"/api/papers/{pid}/reading").json()
    assert ws["state"]["last_page"] is None


def test_recently_read_orders_desc_and_excludes_soft_deleted(client):
    """T2 研究主页：最近阅读按 last_read_at 降序，排除软删除与未读论文。"""
    from datetime import timedelta

    from app.reading.service import list_recently_read

    with Session(get_engine()) as session:
        newest = _paper(session, "Newest Read")
        older = _paper(session, "Older Read")
        never = _paper(session, "Never Read")
        deleted = _paper(session, "Deleted Read")
        base = datetime(2026, 9, 1, tzinfo=timezone.utc)
        session.add(PaperReadingState(paper_id=newest.id, status="reading", last_read_at=base + timedelta(hours=3)))
        session.add(PaperReadingState(paper_id=older.id, status="read", last_read_at=base + timedelta(hours=1)))
        session.add(PaperReadingState(paper_id=never.id, status="unread"))
        session.add(PaperReadingState(paper_id=deleted.id, status="read", last_read_at=base + timedelta(hours=5)))
        deleted.is_deleted = True
        session.add(deleted)
        session.commit()

        rows = list_recently_read(session, limit=5)

    assert [row["id"] for row in rows] == [newest.id, older.id]
    assert rows[0]["title"] == "Newest Read"
    assert rows[0]["status"] == "reading"
    assert rows[0]["last_read_at"] is not None
    assert all(row["title"] != "Deleted Read" for row in rows)


def test_recently_read_limit_caps_results(client):
    """T2 研究主页：limit 截断结果数量。"""
    from datetime import timedelta
    ...
    with Session(get_engine()) as session:
        base = datetime(2026, 9, 1, tzinfo=timezone.utc)
        papers = [_paper(session, f"Recent {i}") for i in range(4)]
        for index, paper in enumerate(papers):
            session.add(
                PaperReadingState(paper_id=paper.id, status="read", last_read_at=base + timedelta(hours=index))
            )
        session.commit()

        rows = list_recently_read(session, limit=2)

    assert len(rows) == 2
    assert rows[0]["title"] == "Recent 3"


def test_recently_read_api_returns_rows_and_validates_limit(client):
    """T2 研究主页：/api/reading/recently-read 默认值与 limit 校验（1..50）。"""
    from datetime import timedelta

    with Session(get_engine()) as session:
        paper = _paper(session, "API Recent")
        pid = paper.id
        base = datetime(2026, 9, 1, tzinfo=timezone.utc)
        session.add(PaperReadingState(paper_id=pid, status="reading", last_read_at=base))
        session.commit()

    resp = client.get("/api/reading/recently-read")
    assert resp.status_code == 200
    body = resp.json()
    assert [row["id"] for row in body] == [pid]
    assert body[0]["last_read_at"] is not None
    assert "authors" in body[0] and body[0]["authors"] == ["Alice"]

    resp = client.get("/api/reading/recently-read", params={"limit": 1})
    assert resp.status_code == 200

    for bad_limit in (0, -1, 51, 999):
        resp = client.get("/api/reading/recently-read", params={"limit": bad_limit})
        assert resp.status_code == 422, f"limit={bad_limit} should be 422"

...
from app.knowledge.recommend import OPENALEX, DiscoveryError, search_related
...
@respx.mock
def test_search_related_network_error_is_distinct_from_empty():
    respx.get(OPENALEX).mock(side_effect=httpx.ConnectError("offline"))
    with pytest.raises(DiscoveryError, match="检查网络"):
        search_related("anything")


@respx.mock
def test_search_related_http_error_is_distinct_from_empty():
    respx.get(OPENALEX).mock(return_value=httpx.Response(503))
    with pytest.raises(DiscoveryError, match="暂时不可用"):
        search_related("anything")

...
@pytest.fixture
def local_token(env):
    # reset_token_cache 对齐 test_local_token.py：lru_cache 进程级共享，
    # 跨测试的 tmp 数据目录变化必须重新解析 token。
...
def _create_tool_skill(client) -> int:
    res = client.post(
        "/api/skills",
        json={"name": "echo-tool", "type": "tool", "body": "print('ran', user_input)"},
    )
    assert res.status_code == 200
    return res.json()["id"]


def test_skills_api_run_tool_with_token(client, local_token):
    sid = _create_tool_skill(client)
    res = client.post(
        f"/api/skills/{sid}/run",
        json={"input": "hi"},
        headers={"X-Local-Token": local_token.get_or_create_token()},
    )
    assert res.status_code == 200
    assert res.json()["ok"] is True
    assert "ran hi" in res.json()["stdout"]


def test_skills_api_run_without_token_is_403(client, local_token):
    sid = _create_tool_skill(client)
    res = client.post(f"/api/skills/{sid}/run", json={"input": "hi"})
    assert res.status_code == 403
    assert res.json()["detail"] == "local token required"


def test_skills_api_run_with_wrong_token_is_403(client, local_token):
    sid = _create_tool_skill(client)
    res = client.post(
        f"/api/skills/{sid}/run",
        json={"input": "hi"},

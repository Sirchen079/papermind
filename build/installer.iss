...
WizardStyle=modern
SetupIconFile=assets\papermind.ico
...
// Never replace an existing destination or a previous migration backup.
...
begin
  ...
  if DirExists(NewData) then begin
    Log('保留现有用户数据，不以旧安装目录覆盖：' + NewData);
    exit;
  end;

...
import { Paper, ThesisChapter, ThesisProject, ThesisWorkspace, type ChapterDraft } from "../api";
import {
...
export default function ThesisWorkspacePanel({ papers, workspace, loading, onRefresh, onOpenPaper }: Props) {
  ...
  const [paperRetry, setPaperRetry] = useState(0);
  useEffect(() => {
    let alive = true;
    setPaperSearching(true);
    setPaperSearchError("");
    const timer = window.setTimeout(() => {
      api.listPapers(50, paperOffset, paperQuery.trim() || undefined)
        .then(page => { if (alive) { setPaperResults(page.items); setPaperTotal(page.total); } })
        .catch(e => { if (alive) { setPaperResults([]); setPaperSearchError(e.message); } })
        .finally(() => { if (alive) setPaperSearching(false); });
    }, 200);
    return () => { alive = false; window.clearTimeout(timer); };
  }, [paperQuery, paperOffset, paperRetry]);
  ...
  // P13 项目详情 tab：项目信息 / 实验列表。
  const [detailTab, setDetailTab] = useState<"info" | "experiments">("info");
  ...
  const validChapterParentOptions =
    selectedProject && selectedChapter
      ...
      : selectedProject
        ...
        : [];
  const selectedPaper = linkForm.paper_id ? [...paperResults, ...data.papers].find((paper) => paper.id === Number(linkForm.paper_id)) ?? null : null;
  const filteredPapers = paperSearching ? [] : paperResults;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .listChapterDrafts(selectedChapter.id)
      .then((versions) => {
        if (cancelled) return;
        setChapterDrafts(versions);
        setDraftVersionId(versions[0]?.id ?? null);
      })
      .catch(() => {
        if (!cancelled) setChapterDrafts([]);
      });
    return () => {
      cancelled = true;
    };
  ...
  // P10.5：调 LLM 生成草稿并追加为新版本。
  async function generateDraft() {
    if (!selectedChapter || generatingDraft) return;
    setGeneratingDraft(true);
    try {
      const draft = await api.generateChapterDraft(selectedChapter.id);
      const versions = await api.listChapterDrafts(selectedChapter.id);
      setChapterDrafts(versions);
      setDraftVersionId(draft.id);
      toast.success("草稿已生成，引用以 [@citation_key] 占位。");
    } catch (err: any) {
  ...
  async function copyDraft() {
    const draft = chapterDrafts.find((d) => d.id === draftVersionId);
    if (!draft) return;
    try {
        ...
        <button
          onClick={() => {
            if (selectedChapterId != null) {
              window.location.href = api.thesisBibtexUrl({ chapter_id: selectedChapterId });
            }
          }}
          className="btn-ghost py-1 text-xs"
          disabled={selectedChapterId == null}
          title="导出该章节（含子章节）挂载论文的 .bib，key 与草稿占位一致"
        >
          导出章节 .bib
        </button>
        <button
          onClick={() => {
            if (selectedProjectId != null) {
              window.location.href = api.thesisBibtexUrl({ project_id: selectedProjectId });
            }
          }}
          className="btn-ghost py-1 text-xs"
          disabled={selectedProjectId == null}
          title="导出该项目（含子项目与子树内章节）挂载论文的 .bib"
        >
          导出项目 .bib
        </button>
          ...
          <section className="space-y-3 rounded-lg border p-3 border-[var(--border)]">
              ...
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <div className="flex rounded-lg border p-1 border-[var(--border)]">
                    <button
                      type="button"
                      className={detailTab === "info" ? "btn-primary py-0.5 text-xs" : "btn-ghost py-0.5 text-xs"}
                    ...
                    <button
                      type="button"
                      className={detailTab === "experiments" ? "btn-primary py-0.5 text-xs" : "btn-ghost py-0.5 text-xs"}
                ...
                {detailTab === "experiments" ? (
                  <ExperimentsPanel projectId={selectedProject.id} papers={papers} />
                ) : (
              <div className="space-y-2">
            ...
            {selectedChapter && (
              <div className="space-y-2 border-t pt-3 border-[var(--border)]">
                ...
                <div className="space-y-2 border-t pt-3 border-[var(--border)]">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="label flex-1">章节草稿</span>
                    {chapterDrafts.length > 1 && (
                      <select
                        className="input max-w-56 py-1 text-xs"
                        value={draftVersionId ?? ""}
                        onChange={(e) => setDraftVersionId(Number(e.target.value))}
                      >
                        {chapterDrafts.map((draft, index) => (
                          <option key={draft.id} value={draft.id}>
                            {index === 0 ? "最新" : `历史 ${chapterDrafts.length - index}`}
                    ...
                    <button onClick={generateDraft} className="btn-primary py-1 text-xs" disabled={generatingDraft}>
                      {generatingDraft ? "生成中…" : chapterDrafts.length > 0 ? "重新生成草稿" : "生成草稿"}
                    </button>
                    {draftVersionId != null && selectedChapter && (
                      <a
                        className="btn-ghost py-1 text-xs"
                        href={api.chapterDraftMarkdownUrl(selectedChapter.id, draftVersionId)}
                        download
                        title="下载当前版本的草稿 Markdown（文件名含章节标题）"
                      >
                        下载 Markdown
                      </a>
                    )}
                    <button onClick={copyDraft} className="btn-ghost py-1 text-xs" disabled={!draftVersionId}>
                      复制
                    </button>
                  </div>
                  {draftVersionId == null ? (
                    <p className="text-xs text-faint">
                      尚未生成草稿。生成会综合本章节挂载论文（按角色分组）的摘要与矩阵，以 [@citation_key] 占位引用。
                    </p>
                  ) : (
                    <pre
                      className="max-h-96 overflow-auto whitespace-pre-wrap rounded-lg p-3 text-xs"
                      style={{ backgroundColor: "var(--surface-2)", whiteSpace: "pre-wrap" }}
                    >
                      {chapterDrafts.find((draft) => draft.id === draftVersionId)?.content ?? ""}
                    </pre>
                  )}
                </div>
              </div>
        ...
        <div className="space-y-4">
          <section className="space-y-3 rounded-lg border p-3 border-[var(--border)]">
            ...
            <input
              ...
              placeholder="搜索论文"
              value={paperQuery}
              onChange={(e) => { setPaperQuery(e.target.value); setPaperOffset(0); }}
            />
            <p className="text-xs text-muted">搜索整个论文库，不受论文库页面筛选限制。</p>
            {paperSearchError && <div role="alert" className="text-sm text-red-600">{paperSearchError} <button className="btn-ghost" onClick={() => setPaperRetry(n => n + 1)}>重试搜索</button></div>}
            <div className="max-h-44 space-y-1 overflow-auto">
              {filteredPapers.length === 0 ? (
                <p className="text-sm text-muted">
                  {paperSearching ? "正在搜索…" : paperSearchError ? "搜索未完成。" : "没有匹配的论文。"}
                </p>
              ) : (
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <span>全库匹配 {paperTotal} 篇 · 第 {Math.floor(paperOffset / 50) + 1} 页</span>
              <button className="btn-ghost" disabled={paperSearching || paperOffset === 0} onClick={() => setPaperOffset(n => Math.max(0, n - 50))}>上一页论文</button>
              <button className="btn-ghost" disabled={paperSearching || !!paperSearchError || paperOffset + 50 >= paperTotal} onClick={() => setPaperOffset(n => n + 50)}>下一页论文</button>
            </div>
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              <select
                className="input"

export type GraphMode = "paper" | "concept" | "claims";
...
export function nodeVisualStyle(mode: GraphMode, theme: GraphTheme, type: string | null | undefined) {
  if (mode === "paper") {
    return {
      ...
      backgroundColor: theme === "dark" ? "#272e2d" : "#fffefa",
      borderColor: theme === "dark" ? "#697b79" : "#a6b5b4",
      textColor: theme === "dark" ? "#e3e9e5" : "#293c43",
      focusColor: theme === "dark" ? "#8bb9e6" : "#3563a2",
    };
  }
  if (mode === "claims") {
    return {
      shape: "round-rectangle" as const,
      backgroundColor: theme === "dark" ? "#302c32" : "#fbf7f3",
      borderColor: theme === "dark" ? "#9987a5" : "#aa96ac",
      textColor: theme === "dark" ? "#e5dde8" : "#54455c",
      focusColor: theme === "dark" ? "#bea7cc" : "#816190",
    };
  }
  ...
  return {
    shape: "round-rectangle" as const,
    backgroundColor: theme === "dark" ? "#25302e" : "#f5f9f7",
    borderColor: conceptColor(key),
    textColor: theme === "dark" ? "#dce8e3" : "#314c46",
...
export function parseGraphParamsFromHash(hash: string): GraphParams {
  ...
  const mode: GraphMode =
    rawMode === "concept" ? "concept" : rawMode === "claims" ? "claims" : "paper";
...
export function buildGraphHash(mode: GraphMode, minPapers: number) {
  ...
  if (mode === "concept") params.set("min_papers", String(Math.max(1, minPapers)));
...
export function modeLabel(mode: GraphMode) {
  return mode === "paper" ? "论文图谱" : mode === "claims" ? "论断图" : "概念图谱";
}

export function modeHint(mode: GraphMode) {
  if (mode === "paper") {
    return "论文节点表示文献，连线表示共享概念，适合找核心文献和相关论文。";
  }
  if (mode === "claims") {
    return "论断节点来自 AI 抽取或手动添加，连线表示支持 / 矛盾 / 延伸关系，适合梳理学术争论。";
  }
  return "概念节点表示研究主题，连线表示共现或上下位关系，适合梳理方向脉络。";
}

export function modeAccent(mode: GraphMode) {
  return mode === "paper" ? "#2563eb" : mode === "claims" ? "#dc2626" : "#047857";
}

// P12 论断图：三类关系的着色与中文标签。
export const CLAIM_EDGE_COLORS: Record<string, string> = {
  supports: "#16a34a",
  contradicts: "#dc2626",
  extends: "#2563eb",
};

export const CLAIM_EDGE_LABELS: Record<string, string> = {
  supports: "支持",
  contradicts: "矛盾",
  extends: "延伸",
};

export const CLAIM_RELATION_ALL = ["supports", "contradicts", "extends"] as const;
...
export function claimEdgeColor(type: string | null | undefined) {
  return CLAIM_EDGE_COLORS[type || ""] ?? "#94a3b8";
}
...
export function nodeMetrics(label: string, count: number, mode: GraphMode) {
  ...
  const minWidth = mode === "paper" ? 176 : 116;
  const maxWidth = mode === "paper" ? 294 : 226;
  const textScale = 14;
  const width = Math.ceil(
    Math.min(maxWidth, Math.max(minWidth, longest * textScale + 30 + Math.min(count, 8))),
  );
  const minHeight = mode === "paper" ? 56 : 42;
  const maxHeight = mode === "paper" ? 122 : 102;
  const lineHeight = 21;
  const height = Math.ceil(Math.min(maxHeight, Math.max(minHeight, 24 + lines.length * lineHeight)));

  return {
    width,
    height,
    textMaxWidth: Math.max(80, width - 26),
  };
}
...
export function normalizeNodes(data: GraphDataLike): GraphNode[] {
  ...
  return (data?.nodes ?? []).map((node) => ({
    id: node.id,
    label: node.label ?? nodeLabel(node),
    ...
    paperTitle: node.paper_title ?? null,
    text: node.text ?? null,
